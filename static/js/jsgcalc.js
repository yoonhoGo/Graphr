...
function JSgCalc(element) {
  ...
  this.getEquation = function(lineid) {
    if (this.lines[lineid]) return this.lines[lineid].equation
    return false
  }

  this.getColor = function(lineid) {
    if (this.lines[lineid]) return this.lines[lineid].color
    return '#000000'
  }
  ...
  this.drawEquation = function(equation, color, thickness) {
    ...
    if (!this.calccache[equation]) this.calccache[equation] = new Object()
    ...
    for (var i = 0; i < maxxval; i += inverseQuality) {
      ...
      if (ypos >= this.height * -1 && ypos <= this.height * 2) {
        if (lineExists > 1) this.ctx.beginPath()
  ...
  this.drawLabel = function(xval, yval, text, color) {
    ...
    if (ypos - 4 < this.charHeight) ypos += this.charHeight * 2
    var textwidth = this.ctx.measureText(text).width
    if (xpos - 4 < textwidth) xpos += textwidth + 3
  ...
  this.drawIntersect = function(equation1, color, x) {
    ...
    var answer = false
    for (i in this.lines) {
      if (this.getEquation(i) == equation1) continue

      var tempanswer = Calc.getIntersection(
        equation1,
        this.getEquation(i),
        xpos,
        50 / scale.x
      )
      if (tempanswer === false) continue
  ...
  this.drawDerivative = function(equation, color, x) {
    ...
    var textwidth = this.ctx.measureText(text).width
    if (pos.x - 4 < textwidth) pos.x += textwidth + 3
  ...
  this.drawTrace = function(equation, color, xval) {
    if (typeof equation === 'boolean') return
  ...
  this.drawGrid = function() {
    ...
    for (i = 0; i < this.maxgridlines.x; i++) {
      ...
      if (
        jsgui.gridlines === 'normal' ||
        (jsgui.gridlines === 'less' &&
      ...
      if (currx != 0) {
        ...
        else if (xpos - xtextwidth * 0.5 < 0) xpos = xtextwidth * 0.5 + 1
    ...
    for (i = 0; i < this.maxgridlines.y; i++) {
      ...
      if (curry != 0) {
        ...
        if (ymainaxis == -1) xaxispos = ytextwidth + 1
        this.ctx.fillText(curry, xaxispos, ypos + 3)
      }
      curry += this.ygridscale
    }
    //Draw the axis
    if (xaxis) this.ctx.fillRect(xaxis - 0.5, 0, 1, this.height)
    if (yaxis) this.ctx.fillRect(0, yaxis - 0.5, this.width, 1)
  }
  ...
  this.checkMove = function(x, y) {
    if (x === this.prevDrag.x && y === this.prevDrag.y) return

    var scale = this.getScale()
    if (this.mousebutton === 1) {
      if (jsgui.currtool === 'zoombox' || jsgui.currtool === 'zoombox_active') {
        //ZOOM BOX
        this.draw()
    ...
    } else if (jsgui.currtool === 'trace') {
      //TRACE
      this.draw()
      this.drawTrace(
        this.getEquation(jsgui.currEq),
        this.getColor(jsgui.currEq),
  ...
  this.initCanvas = function() {
    if (this.graph.getContext) {
      ...
      $('#graph_wrapper').width(
        $('#graph_wrapper').width() -
          $('#sidewrapper').innerWidth() -
          $('#toolbar').innerWidth()
      )
      this.resizeGraph(
        $('#graph_wrapper').innerWidth(),
        $('#graph_wrapper').height()
      )
      this.currCoord = {
        x1: -5 * (this.width / this.height),
        y1: -5,
      ...
      $(window).resize(function() {
        if ($('#sidewrapper').is(':visible'))
          ...
          $('#graph_wrapper').width(
            $('#wrapper').width() - $('#toolbar').innerWidth()
          )
        self.resizeGraph(
          $('#graph_wrapper').width(),
          $('#graph_wrapper').height()

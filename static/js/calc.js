...
function calc() {
  ...
  this.convAngles = function(value) {
    if (this.angles === 'degrees') return value * (Math.PI / 180)
    if (this.angles === 'gradians') return value * (Math.PI / 200)
    return value
  }
  ...
  this.convRadians = function(value) {
    if (this.angles === 'degrees') return (value * 180) / Math.PI
    if (this.angles === 'gradians') return (value * 200) / Math.PI
    return value
  }
  ...
  this.getRoot = function(equation, guess, range, shifted) {
    ...
    if (guess > -0.1 && guess < 0.1 && shifted != true) {
      ...
      dump(answer)
      if (answer !== false) return answer + 5
      return false
    }

    if (!range) var range = 5
  ...
  this.getDerivative = function(f, xval) {
    ...
    var x = xval
    if (x > 1 || x < -1) var h = Math.sqrt(this.eps) * x
    else var h = Math.sqrt(this.eps)
